...
export default function WayDetail({ page }: any){
  ...
  return <main>
    ...
    <section
      className="editorial"
      style={{backgroundImage:`url('/images/${page.slug}.svg')`}}
    >
    ...
    <section
      className="article article-layered"
      style={{backgroundImage:`url('/images/${page.slug}.svg')`}}
    >
      <article className="article-card">
        <h2>{page.title}</h2>
        {paragraphs(page.body).map((p:string,i:number)=>
          <p key={i} className={i===0 ? "quote" : ""}>{p}</p>
        )}
      </article>
    </section>
  </main>
