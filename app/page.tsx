  ...
  return <main>
    ...
    <section className="hero">
      <div className="hero-inner">
        <div className="version">Im v3-11</div>
        <div className="eyebrow">WORD · WORSHIP · GROWTH · SENDING</div>
        <h1>하나님은 사랑이십니다.<br/>하나님은 우리와<br/><span className="gold">함께하십니다.</span></h1>
        <p>말씀 위에 세워지고, 예배로 충만해지며, 성장으로 성숙해지고, 세상으로 파송되는 공동체입니다.</p>
        <div className="hero-actions"><Link className="btn primary" href="/way">임마누엘의 길</Link><Link className="btn ghost" href="/services">교회 서비스</Link></div>
      </div>
    </section>
    ...
    <section className="section dark"><div className="wrap"><h2 className="section-title">세상을 이롭게 하는 사람</h2><p className="section-lead">우리는 자기 삶의 자리에서 가정과 조직과 교회와 사회를 이롭게 하는 제자를 세웁니다.</p><div className="grid">{["탁월함","섬김","회복","치유","번영","파송"].map((x,i)=><div className="card" key={x}><small>LOVE {i+1}</small><h3>{x}</h3><p>사랑은 감정에 머물지 않고 실제 유익을 흘려보내는 힘입니다.</p></div>)}</div></div></section>
    <footer className="footer"><b>IMMANUEL CHURCH</b><br/>God is love. God is with us.</footer>
  </main>
}
